  ...
  constructor (args) {
    ...
    poller(pollerOptions)
      ...
      .then((lastTimestamp) => {
        ...
        oplog.find(query, options).stream().on('data', routeEvent.bind(null, this)).on('end', () => args.log('Tailable cursor was closed'))
      })
      .catch((errors) => {
 ...
 * @return {Promise<Collection>}    A promise resolving to the oplog collection
 */
function getOplogCollection (db) {
  return new Promise((resolve, reject) => {
    db.collection('oplog.rs', {strict: true}, (error, oplog) => {
      if (error) {
        db.collection('oplog.$main', {strict: true}, (error2, mainOplog) => {
          if (error2) return reject(new Error('Could not find oplog collection. Make sure mongodb is configured for replication'))
          resolve(mainOplog)
        })
      } else {
        resolve(oplog)
